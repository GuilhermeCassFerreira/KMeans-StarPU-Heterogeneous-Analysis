...
void assign_point_to_cluster_cuda(void *buffers[], void *cl_arg) {
    ...
    int threads = 256, blocks = (npoints + threads - 1) / threads;
    ...
    double *partial_sums, int *partial_counts)
{
    ...
    for (int i = tid; i < total_vals; i += blockDim.x) s_sums[i] = 0.0;
    ...
    for (int i = tid; i < total_vals; i += blockDim.x)
        if (abs(s_sums[i]) > 1e-9) atomicAdd(&partial_sums[i], s_sums[i]);
    for (int i = tid; i < K; i += blockDim.x)
        if (s_counts[i] > 0) atomicAdd(&partial_counts[i], s_counts[i]);
}
...
void calculate_partial_sums_cuda(void *buffers[], void *cl_arg) {
    ...
    if (shared_mem_size > cached_shared_mem_limit)
        fprintf(stderr, "[KMeans CUDA] WARN: Shared memory insuficiente.\n");
    cudaStream_t stream = starpu_cuda_get_local_stream();
...
__global__ void redux_double_init_kernel(double *dst, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    for (int i = idx; i < n; i += blockDim.x * gridDim.x) dst[i] = 0.0;
}
__global__ void redux_double_reduce_kernel(double *dst, const double *src, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    for (int i = idx; i < n; i += blockDim.x * gridDim.x) dst[i] += src[i];
}
void redux_double_init_cuda(void *buffers[], void *cl_arg) {
    ...
    cudaStream_t stream = starpu_cuda_get_local_stream();
    int t=256, b=min((n+t-1)/t,1024);
    redux_double_init_kernel<<<b,t,0,stream>>>(dst,n);
...
void redux_double_reduce_cuda(void *buffers[], void *cl_arg) {
    ...
    cudaStream_t stream = starpu_cuda_get_local_stream();
    int t=256, b=min((n+t-1)/t,1024);
    redux_double_reduce_kernel<<<b,t,0,stream>>>(dst,src,n);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaStreamSynchronize(stream));
}
__global__ void redux_int_init_kernel(int *dst, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    for (int i = idx; i < n; i += blockDim.x * gridDim.x) dst[i] = 0;
}
...
void redux_int_init_cuda(void *buffers[], void *cl_arg) {
    ...
    cudaStream_t stream = starpu_cuda_get_local_stream();
    int t=256, b=min((n+t-1)/t,1024);
    redux_int_init_kernel<<<b,t,0,stream>>>(dst,n);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaStreamSynchronize(stream));
}
void redux_int_reduce_cuda(void *buffers[], void *cl_arg) {
    ...
    cudaStream_t stream = starpu_cuda_get_local_stream();
    int t=256, b=min((n+t-1)/t,1024);
    redux_int_reduce_kernel<<<b,t,0,stream>>>(dst,src,n);
    CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaStreamSynchronize(stream));
}
...
__global__ void clean_buffers_cuda_kernel(double *partial_sums, int *partial_counts, int total_doubles, int K) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < total_doubles) partial_sums[idx] = 0.0;
    if (idx < K)             partial_counts[idx] = 0;
}
void clean_buffers_cuda(void *buffers[], void *cl_arg) {
    ...
    int threads = 256, blocks = (total_doubles + threads - 1) / threads;
    cudaStream_t stream = starpu_cuda_get_local_stream();
    clean_buffers_cuda_kernel<<<blocks, threads, 0, stream>>>(partial_sums, partial_counts, total_doubles, K);
    cudaStreamSynchronize(stream);
}

/* UPDATE: buffers[0]=sums(R) [1]=counts(R) [2]=centroids(RW) [3]=converged(RW) */
...
void update_centroids_cuda(void *buffers[], void *cl_arg) {
    ...
    // Copia centróides antigos D->H
    double *old_c = (double*)malloc(total_values * sizeof(double));
    CUDA_CHECK(cudaMemcpyAsync(old_c, centroids, total_values * sizeof(double), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));

    // Calcula novos
    int threads = 256, blocks = (K + threads - 1) / threads;
    update_centroids_cuda_kernel<<<blocks, threads, 0, stream>>>(partial_sums, partial_counts, centroids, K, dimensions);
    CUDA_CHECK(cudaStreamSynchronize(stream));

    // Copia novos D->H
    double *new_c = (double*)malloc(total_values * sizeof(double));
    CUDA_CHECK(cudaMemcpyAsync(new_c, centroids, total_values * sizeof(double), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));

    int changed = 0;
    for (int i = 0; i < total_values; ++i) {
        if (new_c[i] != old_c[i]) { changed = 1; break; }
    }
    if (changed == 0) {
        conv[0] = 1;
        conv[1] = current_iter;
