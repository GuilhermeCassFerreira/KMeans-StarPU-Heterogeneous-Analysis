...
void assign_point_to_cluster_cuda(void *buffers[], void *cl_arg) {
    ...
    int *converged = (int *)STARPU_VARIABLE_GET_PTR(buffers[3]); // SINALIZADOR
    ...
    assign_point_to_cluster_cuda_kernel<<<blocks, threads, 0, stream>>>(
        points_values, centroids, K, dimensions, npoints, nearestClusterIds, converged);
...
void calculate_partial_sums_cuda(void *buffers[], void *cl_arg) {
    ...
    int *converged = (int *)STARPU_VARIABLE_GET_PTR(buffers[4]); // SINALIZADOR
...
void clean_buffers_cuda(void *buffers[], void *cl_arg) {
    ...
    int *converged = (int *)STARPU_VARIABLE_GET_PTR(buffers[2]); // SINALIZADOR
    ...
    // CUDA_SYNC REMOVIDO PARA ASSINCRONIA TOTAL DO STARPU
}

/* --- VETO ASSÍNCRONO DE CONVERGÊNCIA --- */

__global__ void setup_converged_flag(int *converged) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (*converged == 0) *converged = 2; // Assume que convergiu temporariamente
    }
}
...
__global__ void update_centroids_cuda_kernel(double *partial_sums, int *partial_counts, double *centroids, int K, int dimensions, int *converged) {
    if (*converged == 1) return; // Já havia convergido de forma permanente
    ...
    if (c < K) {
        if (partial_counts[c] > 0) {
            ...
            // VETO! Se moveu além da tolerância, desfaz a suposição
            if (dist >= 1e-6) {
                *converged = 0;
            }
        }
...
__global__ void finalize_converged_flag(int *converged) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        if (*converged == 2) *converged = 1; // Ninguém vetou! Consolida a convergência
    }
}
...
void update_centroids_cuda(void *buffers[], void *cl_arg) {
    ...
    // Pipeline assíncrono de 3 fases no mesmo stream para evitar syncs com a CPU
    setup_converged_flag<<<1, 1, 0, stream>>>(converged);
    update_centroids_cuda_kernel<<<blocks, threads, 0, stream>>>(partial_sums, partial_counts, centroids, K, dimensions, converged);
    finalize_converged_flag<<<1, 1, 0, stream>>>(converged);
    // CUDA_SYNC REMOVIDO PARA ASSINCRONIA TOTAL DO STARPU
}
